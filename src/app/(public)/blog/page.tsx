...
export default function BlogPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white overflow-x-hidden">
      ...
      <main className="mx-auto w-full px-4 sm:px-6 md:px-10 lg:px-[150px] pt-6 sm:pt-8 md:pt-12">
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 mb-6 sm:mb-8 items-stretch">
              {/* Left Column - Featured Article */}
              <div className="lg:col-span-2 min-w-0">
                {featuredPost && (
                  <Link href={`/blog/${featuredPost.id}`} className="no-underline h-full block">
                    <article className="flex flex-col overflow-hidden rounded-lg bg-white shadow-md transition-all hover:-translate-y-1 hover:shadow-lg h-full">
                      {/* Image */}
                      <div className="h-52 sm:h-64 md:h-72 lg:h-80 w-full overflow-hidden flex-shrink-0">
                        <img 
                      ...
                      {/* Content */}
                      <div className="flex flex-col gap-2 sm:gap-3 p-3 sm:p-4 md:p-5">
                        <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-black font-outfit leading-tight line-clamp-2 sm:line-clamp-none">
                          {featuredPost.title}
                        </h2>
                        <p className="text-sm sm:text-base text-gray-700 font-outfit leading-relaxed line-clamp-2 sm:line-clamp-3 md:line-clamp-none">
                          {featuredPost.excerpt}
                        </p>
                        {/* Author and Date */}
                        <div className="flex flex-wrap items-center gap-3 sm:gap-6 mt-2 sm:mt-4">
                ...
                <div className="mb-4 sm:mb-6">
                  ...
                  <div className="flex flex-col gap-4">
                    {trendingPosts.map((post) => (
                      <Link key={post.id} href={`/blog/${post.id}`} className="no-underline block">
                        <article className="flex gap-2.5 sm:gap-4 overflow-hidden rounded-lg bg-white shadow-md transition-all hover:-translate-y-1 hover:shadow-lg p-2.5 sm:p-4 min-w-0">
                          {/* Image Thumbnail */}
                          <div className="w-20 h-20 xs:w-24 xs:h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 flex-shrink-0 overflow-hidden rounded-lg">
                            <img 
                ...
                {paginatedPosts.map((post) => (
                  <Link key={post.id} href={`/blog/${post.id}`} className="no-underline h-full">
                    <article className="flex flex-col overflow-hidden rounded-lg bg-white shadow-md transition-all hover:-translate-y-1 hover:shadow-lg h-full">
                      ...
                      {/* Content */}
                      <div className="flex flex-col gap-2 sm:gap-2.5 p-3 sm:p-4 md:p-6 flex-1 min-h-[140px] sm:min-h-[160px] md:min-h-[180px]">
            ...
            {/* Pagination */}
            {totalPages > 1 && (
              <div className="flex justify-center mb-8 sm:mb-12 px-2 overflow-x-auto">
                <Pagination
      ...
      {/* Newsletter Section */}
      <div
        className="mt-2.5 w-full px-0 py-6 sm:py-10 md:py-15 text-center"
        style={{
      ...
      >
        <div className="mx-auto max-w-7xl py-4 sm:py-6 md:py-10 px-4 sm:px-6 md:px-10 lg:px-[150px]">
          <h2 className="mb-2 sm:mb-3 md:mb-4 font-outfit text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-white">
          ...
          <form className="mx-auto flex max-w-2xl justify-center gap-2 sm:gap-2.5 items-stretch sm:items-center flex-col sm:flex-row px-2 sm:px-4 w-full sm:w-auto">
            <input
              type="email"
