    ...
    <div className="flex min-h-screen flex-col bg-white overflow-x-hidden">
      ...
      <section className="w-full bg-white py-4 sm:py-6 md:py-8">
        <div className="px-4 sm:px-6 md:px-10 lg:px-[150px]">
          {/* Breadcrumb - same pattern as agents/[id] */}
          <nav className="py-3 sm:py-4 flex items-center gap-2 text-xs sm:text-sm flex-wrap" aria-label="Breadcrumb">
            ...
            <span className="text-gray-400" aria-hidden="true">&gt;</span>
            <span className="text-gray-600 truncate max-w-[140px] sm:max-w-none" title={news.title}>{news.title}</span>
          </nav>
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center min-w-0">
              ...
              {/* NE in black */}
              <span className="text-3xl xs:text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-black font-outfit uppercase ml-2 sm:ml-3 flex-shrink-0">
                NE
              </span>
              {/* WS in red */}
              <span className="text-3xl xs:text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-red-600 font-outfit uppercase flex-shrink-0">
                WS
              </span>
              {/* Red bar extending from WS */}
              <div className="flex-1 min-w-2 h-1 sm:h-1.5 md:h-2 bg-red-600 ml-2 sm:ml-3 flex-shrink" aria-hidden="true"></div>
            </div>
            
            {/* Category Label */}
            <div className="mt-1">
              <span className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-red-600 font-outfit uppercase break-words">
      ...
      <section className="w-full relative">
        <div className="px-4 sm:px-6 md:px-10 lg:px-[150px]">
          <div className="relative w-full h-[320px] xs:h-[380px] sm:h-[500px] md:h-[600px] lg:h-[700px] overflow-hidden rounded-lg">
            ...
            {/* Dark Gray Overlay - full width on mobile, right side on sm+ */}
            <div className="absolute right-0 bottom-0 left-0 sm:left-auto top-auto sm:top-0 sm:bottom-0 w-full sm:w-1/2 md:w-[45%] bg-gray-800/90 flex flex-col justify-end p-4 pb-5 xs:p-5 sm:p-6 md:p-8 lg:p-10 min-h-[45%] sm:min-h-0">
              {/* Main Headline */}
              <h1 className="text-lg xs:text-xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white font-outfit mb-2 sm:mb-4 md:mb-6 leading-tight line-clamp-3 sm:line-clamp-none break-words">
                {news.title}
              </h1>
              
              {/* Summary Text */}
              <p className="text-xs xs:text-sm sm:text-base md:text-lg text-white font-outfit mb-3 sm:mb-6 md:mb-8 leading-relaxed line-clamp-3 sm:line-clamp-none break-words">
                {summary}
              </p>
              
              {/* Footer Information and Social Icons */}
              <div className="flex flex-col gap-3 xs:flex-row xs:items-center xs:justify-between xs:gap-2 sm:gap-3">
                <div className="text-[10px] xs:text-xs sm:text-sm text-white font-outfit flex-shrink-0">
                  BBCTV News • {formatDate(news.published_at)}
                </div>
                
                {/* Social Media Icons - 44px touch targets on mobile */}
                <div className="flex items-center gap-1 xs:gap-2 sm:gap-3 md:gap-4">
                  {/* Share Icon */}
                  <button 
                    className="w-11 h-11 min-w-[44px] min-h-[44px] sm:w-8 sm:h-8 sm:min-w-0 sm:min-h-0 md:w-10 md:h-10 flex items-center justify-center text-white hover:opacity-80 active:opacity-70 transition-opacity touch-manipulation rounded"
                    aria-label="Share"
                  >
                    <svg className="w-5 h-5 sm:w-5 sm:h-5 md:w-[20px] md:h-[20px]" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  >
                    <svg className="w-5 h-5 sm:w-5 sm:h-5 md:w-5 md:h-5" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Article Body */}
      <section className="w-full bg-white pt-6 sm:pt-8 md:pt-12 pb-2">
        <div className="px-4 sm:px-6 md:px-10 lg:px-[150px] max-w-full">
          <article className="mx-auto max-w-full min-w-0">
            {/* Main Title (repeats from hero) */}
            <h2 className="text-lg xs:text-xl sm:text-3xl md:text-4xl font-bold text-black font-outfit mb-4 sm:mb-6 md:mb-8 leading-tight break-words">
              {news.title}
            </h2>
            ...
            <div className="space-y-4 sm:space-y-6 md:space-y-8">
              {contentSections.map((section, index) => {
                if (section.type === 'heading') {
                  return (
                    <div key={index}>
                      <h3 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-black font-outfit mb-3 sm:mb-4 md:mb-6 break-words">
                        {section.content}
                      </h3>
                    </div>
                  )
                  ...
                  return (
                    <p key={index} className="text-sm sm:text-base md:text-lg text-black font-outfit leading-relaxed break-words">
                      {section.content}
                    </p>
                  )
                }
            ...
            <div className="mt-6 sm:mt-6 md:mt-8 pt-4 sm:pt-6 border-t border-gray-200">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-2">
                <div className="text-sm sm:text-base text-gray-600 font-outfit break-words">
                  <span className="font-semibold">Author:</span> {news.author || 'BBCTV News'}
                </div>
                <div className="text-sm sm:text-base text-gray-600 font-outfit flex-shrink-0">
      ...
      <section className="w-full bg-white py-4 sm:py-6 md:py-8 mb-8 sm:mb-10">
        <div className="px-4 sm:px-6 md:px-10 lg:px-[150px] max-w-full">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-4 md:gap-6 overflow-hidden">
            {/* Row 1, Column 1 */}
            <div className="text-left min-w-0 pb-4 sm:pb-0 border-b border-gray-100 sm:border-b-0 sm:border-none">
              <p className="text-sm sm:text-base md:text-lg text-black font-outfit leading-relaxed break-words">
            ...
            <div className="text-left min-w-0 pb-4 sm:pb-0 border-b border-gray-100 sm:border-b-0 sm:border-none">
              {contentSections.find(s => s.type === 'heading' && s.content.includes('Momentum')) ? (
                <>
                  <h3 className="text-base sm:text-lg md:text-xl font-bold text-black font-outfit mb-3 sm:mb-4 break-words">
