...
export default function BrokerInbox() {
  ...
  const fetchConversationMessages = async (conversationId: number) => {
    ...
    try {
      ...
      const unreadMessages = response.data.filter(
        m => !m.is_read && m.sender_email !== (typeof window !== 'undefined' ? localStorage.getItem('user_email') : null)
      )
  ...
  const handleSendReply = async () => {
    ...
    const customerMessage = conversationMessages.find(
      m => !m.sender_id || m.sender_email !== (typeof window !== 'undefined' ? localStorage.getItem('user_email') : null)
    )
  ...
  return (
    <div className="-m-6 md:-m-4 p-8 min-h-[calc(100vh-var(--header-height,80px))] lg:p-6 md:p-4 md:pt-15 bg-gray-100 font-outfit">
      ...
      {isProcessing && showProcessingBanner && (
        <div className="relative flex items-start gap-4 p-4 mb-5 bg-amber-50 border border-amber-200 rounded-xl">
          <div className="flex items-start gap-3 flex-1">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-amber-700 m-0">
                Your account is currently being processed by our admin team. Your listings won't be visible to users until your account is approved.
              </p>
            </div>
          </div>
          <button
            className="w-8 h-8 rounded-lg border-0 bg-transparent flex items-center justify-center text-amber-600 cursor-pointer transition-all duration-200 hover:bg-amber-100"
        ...
        <div className="flex min-h-[600px] flex-col md:flex-row">
          ...
          <div className="flex w-full flex-shrink-0 flex-col border-b border-gray-200 bg-white md:w-[320px] md:border-b-0 md:border-r">
            ...
            <div className="flex-1 overflow-y-auto">
              {loading ? (
                <div className="px-4 py-6 text-center text-sm text-gray-500">Loading conversations...</div>
              ) : filteredConversations.length === 0 ? (
                <div className="px-4 py-6 text-center text-sm text-gray-500">No conversations found.</div>
              ) : (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          <p className="truncate text-sm font-semibold text-gray-900">{conv.customer_name}</p>
                          {conv.latestMessage && (
                        ...
                        {conv.latestMessage && (
                          <p className="mt-0.5 line-clamp-1 text-xs text-gray-400">{conv.latestMessage.message}</p>
                        )}
                      </div>
                    </button>
                  )
                ...
                <div className="flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4">
                  <div className="flex flex-1 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-gray-900">{selectedConversation.customer_name}</p>
                      ...
                      {conversationMessages.map((msg, idx) => {
                        const isFromBroker =
                          ...
                          (typeof window !== 'undefined' &&
                            (localStorage.getItem('user_id') || localStorage.getItem('broker_id')) === msg.sender_id.toString())

                        return (
                          <div
                            key={`${msg.id}-${idx}-${msg.created_at}`}
                            className={`flex items-start gap-3 ${isFromBroker ? 'justify-end' : 'justify-start'}`}
                          >
                            {!isFromBroker && (
                            ...
                            >
                              ...
                              <p className={`mt-1.5 text-[10px] font-medium ${isFromBroker ? 'text-blue-100' : 'text-gray-400'}`}>
